...
const PENA_DATA = [
  ...
  { globero: 'Morín', num1: '47667', num2: '99705' },
  { globero: 'Pajarón', num1: '08439', num2: '08439' },
...
const LoteriaNavidad = () => {
  ...
  useEffect(() => {
    refreshPrizes();
    const interval = setInterval(refreshPrizes, 30000);
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-center">
                          <span className={`font-mono px-3 py-1 rounded-full ${result.prize1 > 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-black font-bold shadow-md' : 'bg-gray-100'}`}>
                            {result.num1}
                          </span>
                        </TableCell>
                        <TableCell className={`text-center font-semibold ${result.prize1 > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                          {result.prize1 > 0 ? formatCurrency(result.prize1) : '-'}
                        </TableCell>
                        <TableCell className="text-center">
                          {result.num2 ? (
                            <span className={`font-mono px-3 py-1 rounded-full ${result.prize2 > 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-black font-bold shadow-md' : 'bg-gray-100'}`}>
                              {result.num2}
                            </span>
                          ) : (
                        ...
                        <TableCell className={`text-center font-semibold ${result.prize2 > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                          {result.num2 && result.prize2 > 0 ? formatCurrency(result.prize2) : '-'}
                        </TableCell>
                        <TableCell className={`text-right font-bold ${hasPrize ? 'text-green-600 text-lg' : 'text-gray-400'}`}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {searchResult && (
            <div className="text-center py-6 space-y-4">
              <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full text-2xl font-bold ${searchResult.prize > 0 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 text-black shadow-lg animate-pulse' : 'bg-gray-200 text-gray-600'}`}>
                {searchResult.number}
              </div>
