...
import { Search, Download, FileText, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
              ...
              <TableHeader>
                <TableRow className="bg-muted/50">
                  ...
                  {distanceCheckpoints.slice(0, 6).map(cp => (
                    <TableHead key={cp.name} className="font-semibold text-center whitespace-nowrap">
                      {cp.name}
                      {cp.distance_km > 0 && (
                        <div className="text-xs font-normal text-muted-foreground">{cp.distance_km}km</div>
                      )}
                    </TableHead>
                  ))}
